...
for(let key of keys){
    ...
    key.addEventListener("click",()=>{
        ...
        if(value=="clear"){
            ...
            display_input.innerHTML=clean_input(input);
        }else if(value =="="){
            let result=eval(prepareInput(input));

            display_output.innerText=clean_output(result);
        }else if(value=="brackets"){
            if(input.indexOf("(")==-1 || input.indexOf("(") != -1 && input.indexOf(")") != -1 && input.lastIndexOf("(") < input.lastIndexOf(")")){
                input+="(";
            }else if(input.indexOf("(")!=-1 && input.indexOf(")") ==-1 ||input.indexOf("(") ==-1 && input.indexOf(")")!=-1 && input.lastIndexOfindexOf("(") > input.indexOf(")")){
                input+=")";
            }
